...
def main(df, df_adversaril, current_batch):
    ...
    df_new.to_csv(f'counterspeech_adversarial/data/iter{str(next_batch)}/train_labelled.csv', index=False)

if __name__ == "__main__":
    args = parse_args()

    df_1 = pandas.read_csv(f'counterspeech_adversarial/data/{args.current_batch}/train_labelled.csv')
    df_adv = pandas.read_csv(f'counterspeech_adversarial/data/adversarial_data/{args.adversarial_data}')
